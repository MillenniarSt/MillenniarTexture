...
public class ZipFileProcess extends ManagmentFile {
        ...
        if(location.exists() && location.isDirectory()) {
            try {
                ...
                try {
                    ...
                    ConsoleStage.defaultCns.printConsole(new ConsoleLine("MANAGMENTFILE", "Fail to close ZipInputStream - IOException in io.github.MillenniarSt.MillenniarTexture.ManagmentFile.zip", ConsoleLine.WARN, exc));
                }
            } catch(IOException exc) {
                ConsoleStage.defaultCns.printConsole(new ConsoleLine("MANAGMENTFILE", "Fail to zip directory \"" + location.getPath() + "\" to \"" + destination.getPath() + "\" - IOException in io.github.MillenniarSt.MillenniarTexture.ManagmentFile.zip", ConsoleLine.FATAL, exc));
                return voidProgress;
            } catch(Exception exc) {
                ConsoleStage.defaultCns.printConsole(new ConsoleLine("MANAGMENTFILE", "Fail to zip directory \"" + location.getPath() + "\" to \"" + destination.getPath() + "\" - Exception in io.github.MillenniarSt.MillenniarTexture.ManagmentFile.zip", ConsoleLine.FATAL, exc));
                return voidProgress;
            }
        } else {
    ...
    public void execute(boolean dialog) {
        ...
        if(dialog) {
            DialogProcess process = new DialogProcess(this, "Zip io.github.MillenniarSt.MillenniarTexture.Files...");
            process.show();
        }
    }
}
