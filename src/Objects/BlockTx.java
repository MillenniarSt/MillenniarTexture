...
import io.github.MillenniarSt.MillenniarTexture.Objects.Component.ModelElement;
import io.github.MillenniarSt.MillenniarTexture.Objects.Component.ModelFace;
import io.github.MillenniarSt.MillenniarTexture.Objects.Component.ModelRotation;
import io.github.MillenniarSt.MillenniarTexture.Objects.Component.RandomVariant;
import io.github.MillenniarSt.MillenniarTexture.Objects.Component.Texture;
import io.github.MillenniarSt.MillenniarTexture.Objects.Component.Variant;
import io.github.MillenniarSt.MillenniarTexture.Dialog.DialogComboBox;
...
import io.github.MillenniarSt.MillenniarTexture.Settings.Derivation;
...
public class BlockTx extends ObjectTx implements Initializable, ProgrammePath {
	...
	private final ObservableList<String> derivationsTx = FXCollections.observableArrayList();
	...
	private final HashMap<Model, HashMap<String, ModelTexture>> modelTexturesMemory = new HashMap<>();
	private final HashMap<Texture, Image> images = new HashMap<>();
	private final HashMap<Texture, HashMap<Model, ModelTexture>> textureChildren = new HashMap<>();
	
	private static final Texture createTexture = new Texture(null, "Create new Texture");
	private static final Model nullModel = new Model(null, "");
	private static final Texture nullModelTexture = new Texture(null, "");
	...
	public BlockTx(Block block) {
		super(block, "Block", new ImageView(Main.getResource("icon/block")));
		this.block = block;
		images.put(createTexture, Main.getResource("button/add"));
	}
	...
	public void open() {
		if(this.getTab() == null) {
			try {
				...
				FXMLLoader loader = new FXMLLoader(Main.getLayout("Objects/Block"));
				loader.setController(this);
				this.setTab(loader.load());
				this.getTab().setText(block.getName());
				this.getTab().setGraphic(new ImageView(Main.getResource("icon/block")));
				...
				ConsoleStage.defaultCns.printConsole(new ConsoleLine("BLOCK", "Open Block object \"" + block.getName() + "\" successfully", ConsoleLine.DEBUG));
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		importBlockstate.setGraphic(new ImageView(Main.getResource("button/import_icon")));
            ...
            public void updateItem(Texture texture, boolean empty) {
                ...
                if (empty) {
                	...
                	if(texture.getPath() != null) {
                		...
                		imageView.setImage(new Image(Main.getResourceStream("exception/file_not_found"), 32, 32, false, false));
			...
			if(check) {
				...
				if(textureFile != null) {
					try {
						...
						images.put(texture, new Image(Main.getResourceStream("exception/wrong_text"), 32, 32, false, false));
					}
				} else {
					textureImage.setImage(Main.getResource("exception/file_not_found"));
				}
			}
		}
	...
	public void save() {
		...
		if(errors.isEmpty()) {
			...
			if(derivationCombo.getSelectionModel().getSelectedItem().trim().isEmpty())
			...
			try {
				...
				writer.close();
				ConsoleStage.defaultCns.printConsole(new ConsoleLine("BLOCK", "Save blockstate \"" + block.getBlockstate().getValue() + "\" successfully", ConsoleLine.DEBUG));
			} catch(FileNotFoundException exc) {
			...
			for(Model model : block.getBlockstate().getModelsUD()) {
				try {
					...
					if(check) {
						...
						writer.close();
						ConsoleStage.defaultCns.printConsole(new ConsoleLine("BLOCK", "Save model \"" + model.getValue() + "\" successfully", ConsoleLine.DEBUG));
					}
				} catch(FileNotFoundException exc) {
			...
			for(Texture texture : block.getBlockstate().getTexturesUD()) {
				if(texture.getFile() != null) {
					...
					if(check) {
						...
						ConsoleStage.defaultCns.printConsole(new ConsoleLine("BLOCK", "Copy texture \"" + block.getBlockstate().getValue() + "\" successfully", ConsoleLine.DEBUG));
					}
				}
			}
			...
			ConsoleStage.defaultCns.printConsole(new ConsoleLine("BLOCK", "Save Block \"" + block.getName() + "\" successfully", ConsoleLine.INFO));
			this.setSave(true);
		} else {
	...
	public void removeVariant() {
		...
		if(variant != null) {
			if(MSDialog.showConfirm("Delete Variant", "Are you sure to delete this variant?", "Yes", "No").equals("Yes")) {
	...
	public void removeModel() {
		Model model = listModels.getSelectionModel().getSelectedItem();
		if(model != null) {
			if(MSDialog.showConfirm("Delete Model", "Are you sure to delete this model?", "Yes", "No").equals("Yes")) {
	...
	public void removeElement() {
		...
		if(model != null && element != null) {
			if(MSDialog.showConfirm("Delete Element", "Are you sure to delete this model element?", "Yes", "No").equals("Yes")) {
	...
	public void removeTexture() {
		...
		if(texture != null) {
			if(MSDialog.showConfirm("Delete Texture", "Are you sure to delete this texture?", "Yes", "No").equals("Yes")) {
	...
	public void selectTexture() {
		...
		if(texture != null) {
			...
			if(textureFile != null) {
				...
				textureImage.setImage(Main.getResource("exception/file_not_found"));
				images.put(texture, new Image(Main.getResourceStream("exception/file_not_found"), 32, 32, false, false));
			}
		} else {
	...
	public void checkGeneral() {
		...
		if(blockstateField.getText().trim().isEmpty()) {
	...
	public void checkRandomVariant() {
		...
		if(variant != null && random != null && enable) {
			try {
				...
				if(blockstateWeight.getText().trim().isEmpty())
					removeError(blockstateWeight);
				else
	...
	public void checkModel() {
		...
		if(model != null && enable) {
			...
			if(modelParentDerivation.getSelectionModel().getSelectedItem().trim().isEmpty())
			...
			String pathParent;
			if(modelParentCategory.getSelectionModel().getSelectedItem().trim().isEmpty())
			...
			if(modelDerivation.getSelectionModel().getSelectedItem().trim().isEmpty())
		...
		if(texture != null && enable) {
			...
			if(textureFile != null) {
				try {
					...
					ConsoleStage.defaultCns.printConsole(new ConsoleLine("BLOCK", "Fail to read texture \"" + textureFile + "\" - IOException in Block.texture.read", ConsoleLine.ERROR, exc));
					textureImage.setImage(Main.getResource("exception/wrong_text"));
					images.put(texture, new Image(Main.getResourceStream("exception/wrong_text"), 32, 32, false, false));
				}
			} else {
				textureSize.setText("Size: .. x .. pixels");
