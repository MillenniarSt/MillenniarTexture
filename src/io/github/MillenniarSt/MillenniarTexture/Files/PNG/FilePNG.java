...
public class FilePNG extends FileTx implements EventHandler<MouseEvent>, Initializable, ProgrammePath, PixelWriter {
	...
	private static final Background background = new Background(new BackgroundImage(Main.getResource("image/void"),
	...
	public void open(ManagmentStage stage) {
		if(this.getTab() == null) {
			try {
				ConsoleStage.defaultCns.printConsole(new ConsoleLine("FILES", "Starting to open png file \"" + this.getPath() + "\"...", ConsoleLine.DEBUG));
				
				FXMLLoader loader = new FXMLLoader(Main.getLayout("/Files/PNG/TabPNG"));
				loader.setController(this);
				this.setTab(loader.load());
				this.getTab().setText(this.getName());
				this.getTab().setGraphic(new ImageView(Main.getResource("icon/file_png")));
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		pen.setGraphic(new ImageView(Main.getResource("button/pen")));
		rubber.setGraphic(new ImageView(Main.getResource("button/rubber")));
		picker.setGraphic(new ImageView(Main.getResource("button/picker")));
	...
	public void resize() {
		try {
			...
			FXMLLoader loader = new FXMLLoader(Main.getLayout("/Files/PNG/ResizePNG"));
			...
			scene.getStylesheets().add(Settings.getTheme());
