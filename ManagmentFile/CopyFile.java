			...
			try {
				...
				if(location.isDirectory()) {
					ConsoleStage.defaultCns.printConsole(new ConsoleLine("MANAGMENTFILE", "Copying directory \"" + location.getPath() + "\" to \"" + destination.getPath() + "\"", ConsoleLine.DEBUG));
					 ...
					 ConsoleStage.defaultCns.printConsole(new ConsoleLine("MANAGMENTFILE", "Copy directory \"" + location.getPath() + "\" to \"" + destination.getPath() + "\" succefly: " + this.getValue()[0] + " / " + this.getValue()[1] + " file copied", ConsoleLine.INFO));
				} else {
					count = 1;
					...
					ConsoleStage.defaultCns.printConsole(new ConsoleLine("MANAGMENTFILE", "Copy file \"" + location.getPath() + "\" to \"" + destination.getPath() + "\" succefly", ConsoleLine.INFO));
					this.updateProgress(1, 1);
					this.updateValue(new Integer[]{1, 1});
				}
			} catch (NoSuchFileException exc) {
				ConsoleStage.defaultCns.printConsole(new ConsoleLine("MANAGMENTFILE", "Fail to copy file \"" + location.getPath() + "\" to \"" + destination.getPath() + "\" - NoSuchFileException in ManagmentFile.copy", ConsoleLine.ERROR, exc));
				this.updateValue(voidProgress);
				return voidProgress;
			} catch (IOException exc) {
				ConsoleStage.defaultCns.printConsole(new ConsoleLine("MANAGMENTFILE", "Fail to copy file \"" + location.getPath() + "\" to \"" + destination.getPath() + "\" - IOException in ManagmentFile.copy", ConsoleLine.ERROR, exc));
				this.updateValue(voidProgress);
				return voidProgress;
			}
		} else {
