...
public class FilePROPERTIES extends FileTXT implements Initializable {
	...
	private SavePROPERTIES savedPROPERTIES;
	...
	public FilePROPERTIES(String path, SavePROPERTIES saved) {
		super(path, saved, new ImageView(new Image(Main.getApl().getClass().getResourceAsStream("/resource/icon/tree_item/file_properties.png"))));
	...
	public FilePROPERTIES(String path, boolean activated) {
		super(path, activated, new ImageView(new Image(Main.getApl().getClass().getResourceAsStream("/resource/icon/tree_item/file_properties.png"))), "properties");
	...
	public FilePROPERTIES(String path, boolean activated, ArrayList<TextureObject> sources) {
		super(path, activated, new ImageView(new Image(Main.getApl().getClass().getResourceAsStream("/resource/icon/tree_item/file_properties.png"))), sources, "properties");
	...
	public void open(ManagmentStage stage) {
		if(this.getTab() == null) {
			try {
				...
				FXMLLoader loader = new FXMLLoader(getClass().getResource("/Files/TabPROPERTIES.fxml"));
				loader.setController(this);
				this.setTab(loader.load());
				this.getTab().setText(this.getName());
				this.getTab().setGraphic(new ImageView(new Image(Main.getApl().getClass().getResourceAsStream("/resource/icon/tree_item/file_properties.png"))));
				stage.getFilesTabPane().getTabs().add(this.getTab());
				stage.getFilesTabPane().getSelectionModel().select(this.getTab());
				ConsoleStage.defaultCns.printConsole(new ConsoleLine("FILES", "Open PROPERTIES file \"" + this.getPath() + "\" successfully", ConsoleLine.INFO));
	...
	public void removeProperty() {
		if(selected != null) {
			if(MSDialog.showConfirm("Delet Component", "Are you sure to delet this component?", "Yes", "No").equals("Yes")) {
