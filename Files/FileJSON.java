...
public class FileJSON extends FileTXT implements Initializable, ProgrammePath {
	...
	private ComponentTx parentMd;
	private SaveJSON savedJSON;
	
	private ObservableList<String> listParent = FXCollections.observableArrayList("null", "Model", "Blockstate");
	
	public FileJSON(String path, SaveJSON savedJSON) {
		super(path, savedJSON, new ImageView(new Image(Main.getApl().getClass().getResourceAsStream("/resource/icon/tree_item/file_json.png"))));
	...
	public FileJSON(String path, boolean activated) {
		super(path, activated, new ImageView(new Image(Main.getApl().getClass().getResourceAsStream("/resource/icon/tree_item/file_json.png"))), "json");
		this.savedJSON = new SaveJSON(this);
	}
	
	public FileJSON(String path, boolean activated, ArrayList<TextureObject> sources) {
		super(path, activated, new ImageView(new Image(Main.getApl().getClass().getResourceAsStream("/resource/icon/tree_item/file_json.png"))), sources, "json");
		this.savedJSON = new SaveJSON(this);
	}
	...
	public void open(ManagmentStage stage) {
		if(this.getTab() == null) {
			try {
				ConsoleStage.defaultCns.printConsole(new ConsoleLine("JSON", "Starting to open json file \"" + this.getPath() + "\"...", ConsoleLine.INFO));
				...
				FXMLLoader loader = new FXMLLoader(getClass().getResource("/Files/TabJSON.fxml"));
				loader.setController(this);
				this.setTab(loader.load());
				this.getTab().setText(this.getName());
				this.getTab().setGraphic(new ImageView(new Image(Main.getApl().getClass().getResourceAsStream("/resource/icon/tree_item/file_json.png"))));
				stage.getFilesTabPane().getTabs().add(this.getTab());
				stage.getFilesTabPane().getSelectionModel().select(this.getTab());
				ConsoleStage.defaultCns.printConsole(new ConsoleLine("FILES", "Open json file \"" + this.getPath() + "\" successfully", ConsoleLine.INFO));
		...
		try {
			...
			if(typeParentCombo.getSelectionModel().getSelectedItem() == null) {
				...
				if(this.parentMd.build(elements)) {
					MSDialog.showInformation("Build succefly", "Complete Model build without error");
				} else {
				...
				if(this.parentMd.build(elements)) {
					MSDialog.showInformation("Build succefly", "Complete Blockstate build without error");
				} else {
			...
			if(component.getValues() == null) {
				if(component.getKey() != null) {
					if(component.isArray()) {
						input = input + levelStr + "\"" + component.getKeyField().getText() + "\" : [";
						String content = readTree(component, level + 1);
						if(content.charAt(0) == '	') {
							input = input + content + levelStr + "]";
						} else {
							input = input + content + "]";
						}
					} else {
				...
				if(component.getKey() != null) {
					input = input + levelStr + "\"" + component.getKeyField().getText() + "\" : " + "\"" + component.getValueField().getText() + "\"";
				} else {
					input = input + " \"" + component.getValueField().getText() + "\"";
				}
				...
				try {
					...
					component.setValues(Float.valueOf(0f));
				}
			} else if(component.getValues() instanceof Boolean) {
				if(component.getKey() != null) {
					input = input + levelStr + "\"" + component.getKeyField().getText() + "\" : " + component.getValueCombo().getSelectionModel().getSelectedItem();
				} else {
					input = input + " " + component.getValueCombo().getSelectionModel().getSelectedItem();
				}
			...
			if(((ComponentJSON<?>) item).getKeyField().getText().equals("display")) {
				...
				for(TreeItem<String> comp : item.getChildren()) {
					...
					if(key.equals("gui"))
						...
						newItems[2] = null;
					else if(key.equals("thirdperson_righthand"))
						newItems[3] = null;
					else if(key.equals("firstperson_righthand"))
						newItems[4] = null;
					else if(key.equals("firstperson_lefthand"))
						newItems[5] = null;
				}
	...
	protected void setPropertiesArea(VBox propertiesArea) {
		this.propertiesArea = propertiesArea;
	}
	protected ComponentTx getParentMd() {
		return parentMd;
	}
	private void setParentMd(ComponentTx parentMd) {
